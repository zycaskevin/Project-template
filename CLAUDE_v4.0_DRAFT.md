...
**Version**: 4.0 (2025 Best Practices Integration)
...
```python
# Multi-Signal Stage Detection
Signals:
  1. Active Agents (小研/小市 → Planning)
  2. File Patterns (*.feature → SBE, test_*.py → TDD Red)
  3. Git Commit Messages (test(TDD-Red) → TDD Red)
  4. TodoList Status (測試撰寫 → TDD Red)
  5. Recent Keywords (last 5 turns: 'pytest', 'assert' → TDD Red)

Example:
  if active_agents in ['小質', '小程'] and recent_keywords(['test_', 'pytest']):
      current_stage = 'tdd_red'
      compression_threshold = 2000  # Conservative
```
...
## 📊 Performance Metrics (v4.0 vs v3.7)

```yaml
Token Efficiency:
  v3.7: ~10,000 tokens/feature (baseline)
  v4.0: ~6,000 tokens/feature (-40%)
    - Stage-aware compression: -2,000 tokens
    - Enhanced handoffs: -1,000 tokens
    - Memory chain: -1,000 tokens
...
Collaboration Efficiency:
  Agent routing accuracy: 85% → 92% (+8%)
  Conflict resolution rate: 80% → 93% (+16%)
  Cross-project reuse: 0% → 40% (+40%)
```
...
- **v4.0** (2025-11-15): **2025 Best Practices Integration**
  ...
  - **Token 節省**: -40% (10,000 → 6,000 tokens/feature)
  - **品質提升**: Hallucination -75%, Source coverage +50%
