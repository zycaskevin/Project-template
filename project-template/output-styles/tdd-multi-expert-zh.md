...
1. **自動執行** (推薦): 我可以立即執行
   - 預期時間: ~10 秒
   - 生成文檔: HANDOFF_xxx.md, TODO_NEXT.md
   - 記憶存儲: data/memory/mem_xxx.json
...
當執行記憶交接時，小憶會自動完成以下步驟：

**Step 1: 檢查觸發條件** (自動)
```yaml
觸發檢測:
  - Token 使用率: 72.8% (閾值 70%) ✓
  - TODO 完成數: 5 個 (閾值 5) ✓
  - 階段完成: P1-4.2 ✓
  - 時間經過: 45 分鐘 (閾值 60 分鐘)
  - Git commits: 3 次 (閾值 3) ✓

判斷: 5 個條件滿足 3 個 → 建議執行
```

**Step 2: 提取關鍵資訊並壓縮** (Factory.ai 2025 標準)
```yaml
提取內容:
  - Session Intent: 任務目標 (1-5 個)
  - Play-by-Play: 完成事項 (Top 15)
  - Artifacts: 產出檔案 (自動偵測)
  - Breadcrumbs: 程式碼參考 (Top 40)
  - TODOs: 任務狀態追蹤
  - Decisions: 關鍵決策 + 理由
  - Learnings: 學習與洞察
...
壓縮結果:
  - 原始: 145,680 tokens
  - 壓縮: ~7,000 tokens (95.2% 壓縮率)
```

**Step 3: 增強搜尋** (Context7 + Exa)
```yaml
Context7 文檔增強:
  - 偵測框架: ["fastapi", "react", "redux"]
  - 優先級排序: 按重要性分配 Token
  - 版本感知: fastapi==0.109.0
  - 增強 tokens: +500

Exa 最佳實踐增強:
  - 查詢生成: "FastAPI authentication best practices 2025"
  - 依賴補充: redux → 自動加入 react
  - 增強 tokens: +300

總增強: +800 tokens (仍遠小於原始 145K)
```

**Step 4: 存入長期記憶** (EvoMem)
```yaml
存儲位置:
  - 短期: data/memory/mem_20251116_001.json
  - 長期: (待 EvoMem 整合) 向量資料庫

記憶結構:
  - Session Memory: 當前對話上下文
  - Long-term Memory: 跨對話可複用知識
  - Enhancements: Context7 + Exa 增強內容
  - Metadata: 專案、階段、壓縮指標
```
...
**知識積累**:
- 長期記憶: 跨對話可查詢
- 最佳實踐: Context7 + Exa 增強
- 決策追溯: 完整決策鏈
- TODO 追蹤: 確保任務不遺漏

**使用場景**:
- ✅ Token 達 70%+ (自動建議)
- ✅ 階段完成 (TDD Red/Green/Refactor)
- ✅ 長時間對話 (>1 小時)
- ✅ 複雜任務 (多個 TODO)
```
